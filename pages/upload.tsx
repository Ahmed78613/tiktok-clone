...
const Upload: NextPage = () => {
	...
	const uploadVideo = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (fileTypes.includes(selectedFile[0].type)) {
			setWrongFileType(false);
			setIsLoading(true);

			client.assets
				.upload("file", selectedFile[0], {
		...
		<div className="absolute flex w-full h-full left-0 top-[60px] mb-10 pt-10 lg:pt-20 bg-[#F8F8F8] justify-center">
			<div className="bg-white rounded-lg xl:h-[80vh] flex gap-6 flex-wrap justify-between items-center p-14 pt-6 w-[60%]">
									...
									<label className="cursor-pointer ">
										<div className="flex flex-col items-center justify-center h-full">
											<div className="flex flex-col items-center justify-center">
												...
												<p className="text-xl font-semibold">Select upload</p>
											</div>
											...
											<p className="bg-[#F51997] text-center mt-1- rounded text-white text-md font-medium p-2 w-52 outline -none">
												Select a File
											</p>
										</div>
										<input
				...
				<div className="flex flex-col gap-3 pb-10">
					<label htmlFor="caption" className="font-medium text-md">
						Caption
					</label>
					<input
						id="caption"
					...
					<div className="flex gap-6 mt-10">
						...
						<button
							onClick={handlePost}
							type="button"
							className="p-2 font-medium bg-[#F51997] text-white border-gray-300 rounded outline-none text-md w-28 lg:w-44"
						>
							Post
						</button>
					</div>
